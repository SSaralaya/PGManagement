...
function send_email($email, $name, $jwt)
{
  ...
  try {
    ...
    $mail->Body    = "Here is your reset link its is valid up to 5 minutes<br> <a href='http://localhost/PGManagement/PGLogin/reset_password_code.php?token=$jwt&email=$email'>Click here</a>";
    $mail->AltBody = 'http://localhost/PGManagement/PGLogin/reset_password_code.php?token=$jwt&email=$email';
...
if (isset($_POST["r_btn"])) {
  ...
  if (!empty($email)) {
    $res = $maincon->query("SELECT * FROM tblpglogin WHERE emailid='$email'");
    if ($res->num_rows > 0) {
      $row = $res->fetch_array();
      $token = md5(rand());
      if ($maincon->query("UPDATE tblpglogin SET verify_token='$token' WHERE emailid='$email'")) {
        $payload = [
          'verify_token' => $token,
          'email' => $email,
          'exp' => time() + 300
        ...
        if (send_email($email, $row["pgcode"], $jwt)); {
...
if (isset($_SESSION["status"])) {
  ...
  // <div class="alert alert-' . $_SESSION["class"] . '" role="alert">' . $_SESSION["status"] . '</div>';
